...
void frmMain::on_cmdFileOpen_clicked()
{
    if (!m_heightMapMode) {
        ...
        QString fileName  = QFileDialog::getOpenFileName(this, tr("Open"), m_lastFolder,
                                   tr("G-Code files (*.gcode *.nc *.ncc *.ngc *.tap *.txt);;All files (*.*)"));
...
{
    QString fileName = (QFileDialog::getSaveFileName(this, tr("Save file as"), m_lastFolder, tr("G-Code files (*.gcode *.nc *.ncc *.ngc *.tap *.txt)")));
